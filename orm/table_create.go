...
func UpdateTable(db DB, model interface{}, opt *UpdateTableOptions) error {
	return NewQuery(db, model).UpdateTable(opt)
}
...
func newUpdateTableQuery(q *Query, opt *UpdateTableOptions) *updateTableQuery {
...
func (q *updateTableQuery) Clone() queryCommand {
	return &updateTableQuery{
		q:   q.q.Clone(),
		opt: q.opt,
	}
...
func (q *updateTableQuery) Query() *Query {
	return q.q
}

func (q *updateTableQuery) AppendTemplate(b []byte) ([]byte, error) {
	return q.AppendQuery(dummyFormatter{}, b)
}

func (q *updateTableQuery) AppendQuery(fmter QueryFormatter, b []byte) (_ []byte, err error) {
	if q.q.stickyErr != nil {
		return nil, q.q.stickyErr
	}
	if q.q.model == nil {
...
func (q *updateTableQuery) appendSQLType(b []byte, field *Field) []byte {
	if field.UserSQLType != "" {
		return append(b, field.UserSQLType...)
	}
	if q.opt != nil && q.opt.Varchar > 0 &&
...
func (q updateTableQuery) appendFKConstraint(fmter QueryFormatter, b []byte, rel *Relation) []byte {
	if rel.Type != HasOneRelation {
		return b
	}
...
func (q updateTableQuery) appendTablespace(b []byte, tableSpace types.Safe) []byte {
	b = append(b, " TABLESPACE "...)
	b = append(b, tableSpace...)
	return b
}
